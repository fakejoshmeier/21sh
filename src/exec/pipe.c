...
/*   Updated: 2019/09/28 17:17:36 by jmeier           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_launch_pipeline(t_ast *left, t_ast *right)
{
	int		fd[2];
	pid_t	pid_left;
	int		status_right;
	...
	{
		close(fd[0]);
		ft_make_dup2(left->token->str, fd[1], STDOUT_FILENO);
